...
def edit():
    ...
    with engine.connect() as connection:
        connection.execute(text(f'UPDATE invoices SET invoiceNumber={results[1]}, custName="{results[2]}", custAddress="{results[3]}", invoiceDate="{results[4]}", invoiceTotal={results[5]}, invoiceCurrency="{results[6]}", invoiceDescription="{results[7]}" WHERE id={results[0]}'))
...
def confirmEdit(id):
    with engine.connect() as connection:
        result = connection.execute(text(f'SELECT * FROM invoices WHERE id={id}'))
...
def submit():
    ...
    with engine.connect() as connection:
        connection.execute(text(f'INSERT INTO invoices (invoiceNumber, custName, custAddress, invoiceDate, invoiceTotal, invoiceCurrency, invoiceDescription) VALUES ({formResults[0]}, "{formResults[1]}", "{formResults[2]}", "{formResults[3]}", {formResults[4]}, "{formResults[5]}", "{formResults[6]}")'))
...
def delete():
    id = request.form['id']
    with engine.connect() as connection:
        connection.execute(text(f'DELETE FROM invoices WHERE id={id}'))
